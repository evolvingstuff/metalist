    ...
    for (const item of items) {
        if (item.type.indexOf('image') === 0) {
            ...
            reader.onload = async function(event) {
                ...
                console.log('Saving note content after image paste');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function saveAfterInactivity() {
        if (currentEditingNote) {
            ...
            if (currentContent !== lastSentContent) {
                console.log('Saving note after inactivity');
    ...
    // TODO: refactor this it duplicates code and is ugly
...
document.addEventListener('blur', (e) => {
    //TODO: refactor into "make note uneditable" function
    const contentDiv = e.target.closest('.note-content');
    if (contentDiv && currentEditingNote) {
        console.log('Blur: Note lost focus:', contentDiv.textContent);
        const finalContent = contentDiv.innerHTML;
        if (finalContent !== lastSavedContent) {
            console.log('Saving note on document blur');
            saveNoteContent(currentEditingNote, finalContent);
        }
        contentDiv.contentEditable = 'false';
        currentEditingNote.classList.remove('editing');
...
}, true); // Use the capture phase to ensure blur events are caught