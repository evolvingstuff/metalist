...
- `AI Agent Settings…` configures the temporary unmanaged Ollama connection. Only explicit loopback URLs are accepted. The read-only agent may search and read hydrated notes through application-owned tools; no mutation action exists. See `docs/ui/ai-chat.md`.
- `Agent prompts…` inspects all three packaged runtime prompts and saves validated namespace-specific overrides. Reset removes the overrides and resumes using packaged defaults; changes apply to the next agent run and never enter conversation history.
...
- `AI Agent Settings…`: configures the loopback Ollama endpoint used by the read-only agent harness. The chat composer loads installed models and keeps its model and thinking-level selectors immediately left of Send.
- `Agent prompts…`: opens the system/control prompt editor. Template placeholders are validated before saving, and protected namespaces encrypt these overrides through the existing client-state path.
