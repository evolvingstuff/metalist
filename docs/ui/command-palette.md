...
- `AI Agent Settings…` configures the Phase 1 temporary unmanaged Ollama connection. Only explicit loopback URLs are accepted, and no MetaList note data is included in chat prompts. See `docs/ui/ai-chat.md`.
...
- `AI Agent Settings…`: configures the loopback Ollama endpoint for standalone chat. The chat composer loads installed models and keeps its model and thinking-level selectors immediately left of Send.
