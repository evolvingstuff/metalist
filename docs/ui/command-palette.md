...
- `Search suggestion statistics…`: shows the authenticated in-memory daily aggregate used for personalization, including every retained date and tag-credit count. Its `Reset activity` button clears the aggregate after confirmation. It does not collect search text or note content.
- `Reset search suggestion activity…`: clears the same aggregate after confirmation and is searchable with phrases including `reset statistics` and `reset activity`.
