...
- Writing assistants: document mouse-down and click handlers leave events whose composed path includes `grammarly-extension` or `grammarly-desktop-integration` alone, including retargeted shadow-root events. This prevents premature save/deselect and click consumption before a suggestion is applied. Save actions compare the sanitized editor DOM with the last observed content snapshot before skipping a clean note; an external correction without an `input` event is recorded before collapse persistence. This safety check runs at save time, not as a DOM observer or a separate autosave timer. Repeated unchanged observations do not call strict setters or create duplicate saves.
...
- Editor-to-tags focus transfers publish one selection record (note ID, cloned Range, cursor offset). Returning consumes it once; a new transfer expires an unconsumed record. Restore only into its original note, using the offset if its DOM Range was detached. Palette focus/scroll restoration likewise belongs to one open/close cycle.
- Bulk completion uses a pending completion record: `null` means no completion, while `{changed: false}` is a completed operation with no changes. Cleanup consumes that record once. Chat final snapshots and rendered deltas can match earlier server observations; stopping thinking feedback releases only an active timer.
- Ontology typing/dismissal consumes pending navigation flags; cache invalidation clears only an existing cache. Repeated arrow keys at a boundary request no transition. Image request completion must still own its pending cache entry before publishing a blob URL; page cleanup cancels ownership, and request cleanup must preserve the original rejection.
