...
- A literal content hit on any connector-separated segment can also surface a tag, but low-signal glue segments like `and`, `no`, `of`, `the`, `to`, or `up` do not count on their own. Full multi-segment phrase hits rank above single-segment hits, so content like `databricks workspaces` prefers `databricks-workspaces` over either segment tag alone.
- Connector-heavy tags need near-complete literal coverage before they surface from content alone: a tag with `k` significant segments needs at least `k-1` of them to appear in the note text. That means `X-Y-Z` can surface for `X Y`, `Y Z`, or `Y X`, but not for `X`, `Y`, or `Z` alone.
