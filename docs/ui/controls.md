...
| Click a Grammarly suggestion card | Keep the current note in edit mode while the assistant applies the correction. A subsequent ordinary outside click or `Escape` saves and exits as usual. |
...
| Right-click note | Open the note context menu (`Copy Note`, `View Full Screen`, `Paste Sibling Note`, `Paste Child Note`, `Paste Sibling Reference`, `Paste Child Reference`, `Add Sibling Note`, `Add Child Note`, `Delete Note`, `Move Note to Top`, `Fully Expand Note`, `Fully Collapse Note`, `Export Note as HTML`, `Export View as HTML`). Fully expanding/collapsing applies recursively to the selected note and all descendants, including descendants hidden by collapsed ancestors. A dark, non-interactive footer at the bottom always shows the note's locally formatted `Created` and `Updated` date/times. `View Full Screen` is available only outside edit mode; it renders the selected note edge-to-edge with its complete descendant subtree but without ancestors, hides the menu and scroll-to-top controls, and closes with the upper-right `×` or `Esc`. While actively editing that note, `Add Style` opens a connected formatting flyout and `Remove Formatting` performs the same operation as `Cmd/Ctrl+U`. Paste actions appear only when a note has been copied. |
| Open in Floating Window (note menu) | Open that note and its complete descendant subtree in a read-only floating window. The title shows a ↗ reference link that opens the source note in ML3; password titles stay masked. Open multiple windows, drag the title-bar grip or empty space, and resize using the bottom-right handle (or focus the handle and use arrow keys). Windows stay fixed while the background scrolls and survive searches and ML3 tab changes. Saved changes appear live. Links, password/username copying, copyable text, and file downloads remain interactive; note references open another floating window. Close with × or Escape while focused. Windows close on page reload, logout, or namespace navigation. |
...
| Right-click anywhere in a completed AI response bubble (including padding and links) | Choose `Copy Response` to place its raw Markdown on the MetaList note clipboard with `@markdown @llm`, while also writing rendered HTML and raw Markdown to the system clipboard. It can be pasted as a note but not as a reference until inserted. |
| Interact with AI chat while editing a note | Keep the note in edit mode while using the chat input, controls, or transcript. Send saves the current draft without deselecting it and supplies its entire containing top-level tree—including ancestors, siblings, collapsed descendants, and per-note tags—separately from the broader result context, subject to search redaction and AI privacy rules. Previously fetched URL titles are included with permitted notes, including parent URLs. The edited note is explicitly marked when permitted; revealing or editing an excluded note does not share it with AI. Chat receives only selection presence and the blocking reason, so it can explain a blacklist or search restriction accurately. The model interprets the request using both contexts; selecting a note does not automatically narrow every question to it. Explicit reference navigation can still change the view. |
...
- AI Agent Settings… (OpenAI API key, model, cloud privacy, evidence and tagging limits; model and thinking level are selected beside chat Send)
