...
- Namespace launch profiles live as plaintext metadata inside each namespace's main DB, so after a first explicit run you can use `python main.py work` and the sidecar will reuse the saved ports for `work`.
