...
The build uses `python -m build --no-isolation`, so the pinned setuptools/wheel/build tools are actually used. Distribution verification checks runtime resources and rejects obsolete vendor JavaScript as well as missing/stale files. One verified wheel/source-distribution pair is reused by every downstream job. Each clean platform environment installs hash-verified runtime dependencies and then that wheel with `--no-deps`, outside the checkout. Dependency consistency and actual installed CLI/HTTP/HTTPS startup run on Windows, macOS, and Linux at the supported Python endpoints, 3.10 and 3.14. GitHub caches downloaded packages and locked browser tooling, while pip hash verification and the npm lockfile remain mandatory on every clean installation.

Publication requires build, all platform/Python unit jobs, per-OS JavaScript/sanity, every installed-package/update job, browser smoke, and browser soak, using the same tested distributions. Manual dispatch cannot publish. **Before creating any release tag**, the exact commit must already have a successful complete main workflow run. The tag job independently verifies that run's jobs and artifact, then publishes the same files without repeating the matrix. Local checks, a previous commit, or a pending/skipped/canceled/failed job do not meet that gate. The full cross-platform matrix runs once on the main push, not on feature branches. Local macOS validation does not establish Windows/Linux success.
