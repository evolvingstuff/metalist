...
Publication requires build, all platform/Python unit jobs, per-OS JavaScript/sanity, every installed-package/update job, browser smoke, and browser soak, using the same tested distributions. This applies to manual dispatch as well as tags. **Before creating any release tag**, the exact commit must already have a successful complete workflow run. Local checks, a previous commit, or a pending/skipped/canceled/failed job do not meet that gate. Cross-platform runtime changes also require applicable feature-branch CI before merge. Local macOS validation does not establish Windows/Linux success.
