...
`Publish to PyPI` builds one wheel/source-distribution pair and reuses those exact artifacts in every downstream job. Publication requires every applicable job group:

- `Python`: the complete Python unit suite on Windows, macOS, and Linux at the supported Python endpoints, 3.10 and 3.14.
- `JavaScript and sanity`: the complete JavaScript suite and both startup sanity gates once on each operating system. These checks do not vary with the Python minor version, so they are not repeated five times per OS.
- `Package and update`: every operating system at Python 3.10 and 3.14 installs the one verified wheel outside the checkout with hash-locked runtime dependencies, checks dependency consistency, launches two HTTP/HTTPS namespaces, and runs the real managed-uv update/restart.
- `Browser smoke`: Chrome and Firefox on every OS plus Edge over non-loopback verified HTTPS on Windows. Each performs one fresh browser context, two uncached loads per namespace, and the encrypted-login/session-storage-loss scenario.
- `Browser soak`: the same browser/OS coverage performs three fresh contexts and three uncached loads per namespace on main pushes and explicit manual validation on main. It isolates repeated-load transport stability from login correctness while remaining release-blocking.

- Every OS/Python endpoint combination: three passes through all installed JavaScript, CSS, JSON and icon assets over both HTTP and HTTPS, with six persistent connections per transport. Check status, decoded bytes, JavaScript MIME type, and continued socket reuse. TLS probes trust the temporary application's certificate explicitly and verify its hostname. No source-checkout assets can supply missing wheel files.
- Every OS/Python endpoint combination: configure LAN access once, launch the installed CLI again with bind/allowed-host environment overrides removed, and require both namespaces to remain reachable through a non-loopback address over HTTP and certificate-verified HTTPS. The self-update fixture also upgrades from that fresh environment and verifies both namespaces over LAN afterward. These checks cover remembered configuration; carrying environment variables through an update alone does not establish it.
- Every platform/Python combination: isolate the operating system's MetaList update cache, run the real self-update path without relying on a user-global uv for installation, and require the checksum-pinned managed uv executable to exist afterward. Preflight and final offline installation must use the same managed executable.
- Windows, macOS, and Linux on Python 3.12 use the actual Chrome and Firefox executables; Windows additionally uses Microsoft Edge over a non-loopback IPv4 HTTPS address. The Edge job temporarily trusts only its generated certificate in the disposable GitHub-hosted Windows VM’s machine Root store and removes it afterward; it does not disable browser TLS validation. A delivered 503 from the optional PyPI release check is recorded as an expected outage; missing routes, transport failures, and asset failures still fail the gate.
- The published 0.7.1 standalone repair upgrade is retired from release validation. Windows Chrome, Firefox, and Edge remain required in the ordinary installed-package and browser checks; the current managed-uv updater still runs on every OS/Python package leg.
- Retain per-browser results, failure screenshots, launcher output, and backend server logs for every required operating system. Missing Chrome, Firefox, Edge on its required Windows legs, the locked driver, diagnostics, or a required check blocks release. Diagnostic-artifact upload must not add a second failure when an earlier step fails before creating files. No browser may substitute for another, and `continue-on-error` or retry-to-green is forbidden.
...
Do not remove the cross-platform Chrome/Firefox legs or the Windows Edge leg, narrow these asset checks, or decouple publication from their success as part of unrelated work. A release tag still requires successful validation of the exact main commit before tagging. Manual dispatch does not bypass the matrix or replace the required main-push evidence. Dependency caches may reuse downloads, but every Python installation remains hash-verified and every browser driver remains lockfile-resolved with install scripts disabled.

The HTTPS proxy may retry one bodyless `GET`, `HEAD`, or `OPTIONS` after a transport reset before any response begins. It never replays mutating requests or request bodies. A final failure remains a 502 and records the exception type in server logs without disclosing transport details to the browser. This product behavior is distinct from CI retry-to-green, which remains forbidden.
