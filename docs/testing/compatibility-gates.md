...
- Every platform/Python combination runs the complete Python unit suite, JavaScript unit suite, and both startup sanity gates from the same checkout. Platform-specific checks are additive; they never substitute a narrower suite for core behavior on another operating system.
...
- Windows, macOS, and Linux on Python 3.12: the actual Chrome and Firefox executables each load both installed namespaces over loopback HTTP, matching the normal local application route. Three fresh browser contexts each perform three uncached loads per namespace, then exercise password creation, encrypted login, logout, loss of `sessionStorage` identity during login, hydration, and fixture restoration. Require the real module graph, visible application, `data-app-ready`, preserved in-memory tab identity, and no failed requests, startup dependency HTTP errors, or JavaScript exceptions.
- Windows/Python 3.12 additionally runs the identical browser scenario in Microsoft Edge over a non-loopback IPv4 HTTPS address. The job temporarily trusts only its generated certificate in the disposable GitHub-hosted Windows VM’s machine Root store and removes it afterward; it does not disable browser TLS validation. A delivered 503 from the optional PyPI release check is recorded as an expected outage; missing routes, transport failures, and asset failures still fail the gate.
...
- Retain per-browser results and failure screenshots for every required operating system. Missing Chrome, Firefox, Edge on its required Windows legs, the locked driver, diagnostics, or a required check blocks release. No browser may substitute for another, and `continue-on-error` or retry-to-green is forbidden.
