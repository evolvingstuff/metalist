...
- Packaging: `app/version.py` is the single `0.3.3` version source consumed dynamically by `pyproject.toml` and runtime config; the package includes templates/static assets, and installed helper commands include `metalist-mcp`, `metalist-audit-encryption`, `convert-from-legacy.py`, and `generate-lan-cert.sh`.
...
- Namespace switching/launching: `app/services/namespace_switcher.py` lists known namespaces, suggests conflict-free ports, reuses an already-running target with a warm cache when its saved launch profile is unchanged, restarts it when requested ports changed, relaunches the recorded entrypoint (installed CLI or source script), auto-evicts stale listeners on assigned ports, and writes child logs under `~/MetaList/logs/`. Plain `python main.py` orchestration still deliberately restarts all namespaces to pick up current code.
