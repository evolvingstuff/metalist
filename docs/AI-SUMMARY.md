...
- Packaging: `app/version.py` is the single `0.3.7` version source consumed dynamically by `pyproject.toml` and runtime config; the package includes templates/static assets, and installed helper commands include `metalist-mcp`, `metalist-audit-encryption`, `convert-from-legacy.py`, and `generate-lan-cert.sh`.
...
- Namespace launch profiles: `app/server_runtime.py` stores remembered per-namespace HTTP / HTTPS / MCP sidecar ports as plaintext metadata in each namespace's main `*.metalist.db`; the old global `~/MetaList/namespaces.db` registry is ignored.
- Namespace switching/launching: `app/services/namespace_switcher.py` lists known namespaces, suggests conflict-free ports, reuses an already-running target with a warm cache when its saved launch profile is unchanged, restarts it when requested ports changed, relaunches the recorded entrypoint (installed CLI or source script), auto-evicts stale listeners on assigned ports, and writes child logs under `~/MetaList/logs/`. Windows listener discovery and process termination use built-in PowerShell through `app/services/windows_process_control.py`; macOS/Linux retain the POSIX `lsof` and signal path. Plain `python main.py` orchestration still deliberately restarts all namespaces to pick up current code.
...
- `app/encryption_audit.py`: Read-only, fail-closed audit of every encrypted namespace's main/files SQLite payload contract; exposed as `scripts/audit_encrypted_namespaces.py` and installed command `metalist-audit-encryption`. `main.py` and `serve_namespace.py` run it at every startup, printing a large non-blocking warning with per-namespace `PASS`/`FAIL` results so password-driven migrations can still proceed.
...
- Version/migrations: `app/db/migrations.py` owns ordered `PRAGMA user_version` upgrades (current DB version `1`). Passwordless namespaces migrate at startup; encrypted namespaces back up and migrate after successful DEK unwrap. App version and vault version are independent.
...
- Calendar/tabs visibility: `Cmd/Ctrl+/` exposes `Toggle tabs` and `Toggle calendar view`; right-clicking the left/right lanes provides the same visibility toggles. The tabs overlay remains open when hover-triggered `Enter` creates a blank tab, and visible search suggestions stack above it.
...
- Image context menu: right-clicking a rendered image in a note adds image actions (`Copy Image`, `Save Image`, `Zoom Image`, `Open Image in New Tab`) to the note context menu; saved-file image actions only appear in view mode because edit mode shows raw UUID tokens.
