...
- Packaging: `app/version.py` is the single `0.3.6` version source consumed dynamically by `pyproject.toml` and runtime config; the package includes templates/static assets, and installed helper commands include `metalist-mcp`, `metalist-audit-encryption`, `convert-from-legacy.py`, and `generate-lan-cert.sh`.
...
- Namespace commands: `Cmd/Ctrl+/` exposes separate modals for `Switch namespace…`, `Create namespace…`, `Manage namespace ports…`, and `Delete current namespace…`. Switching/creation use `POST /api2/auth/namespaces/open`; port management uses `POST /api2/auth/namespaces/ports` to save launch profiles only, so current-namespace port edits apply on the next launch.
...
- Backup/restore: `app/services/backup_service.py` snapshots the notes DB plus sibling files DB into one versioned `.tar.gz` archive, validates manifest checksums/format on restore, rebuilds the file registry afterward, ignores legacy search-history sidecars, and still restores legacy `.bak` snapshots for compatibility. Same-name restore overwrites normally. Different-name import can create or explicitly overwrite a target namespace; existing targets always retain their own saved launch profile, while new targets receive automatically selected conflict-free ports. Restore UI never edits ports—port changes belong to the separate namespace-port manager.
