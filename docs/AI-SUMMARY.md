...
- Notes schema: `notes.content` + `notes.tags` are persisted; tags are a space-separated string. `notes.updated_at` means note body content changed; tag-only edits, structural relinks, collapse state, encryption rewrites, and no-op saves preserve it.
